...
class TestKnownSchedulesFastPath:
    ...
    async def test_fast_path_refreshes_stale_cache_reference(self):
        """
        If known_schedules points at an older cache row, fast-path should refresh
        the result from the current best course_research_cache entry and persist it.
        """
        entries = [_make_entry("CSE 123", "Shalev, Aaron")]
        prebuilt = _make_prebuilt_response(entries)
        prebuilt.results[0] = prebuilt.results[0].model_copy(update={
            "cache_id": "stale-cache-id",
        ...
        refreshed_row = CourseResearchCacheRow(
            id="fresh-cache-id",
            course_code="CSE 123",
            course_title="Computer Networks",
            professor_name="Shalev, Aaron D",
            normalized_course_code="CSE 123",
            normalized_professor_name="SHALEV, AARON D",
            logistics=CourseLogistics(
                rate_my_professor={
                    "rating": 5,
                    "difficulty": 3,
                    "would_take_again_percent": 100,
                    "url": "https://www.ratemyprofessors.com/professor/3050657",
                },
